    ...
    notifyOnlineSocket() {
        ...
        for (let i = 0, j = this.socketStore.length; i < j; i++) {
            if (this.socketStore[i].userinfo.class_id === this.class_id) {
                ...
                this.socketStore[i].socket.write(SocketEventConvert.teacherGetPhotoToStudent({ 'teacher_id': this.singleSocket.id, 'teacher_name': this.singleSocket.userinfo.name }));
            }
        }
